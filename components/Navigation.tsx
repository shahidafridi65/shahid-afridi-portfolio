						...
						{navItems.map((item, index) => (
							<div key={item.id} className="relative">
								<motion.button
									...
									className={`relative px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 z-10 ${activeSection === item.id
										? "text-white"
				...
				<div className="flex md:hidden items-center justify-center gap-1 mt-3 overflow-x-auto pb-2">
					{navItems.map((item) => (
						<motion.button
							key={item.id}
							onClick={() => scrollToSection(item.id)}
							className={`px-2 py-1.5 rounded-full text-xs font-medium whitespace-nowrap flex-shrink-0 ${activeSection === item.id
